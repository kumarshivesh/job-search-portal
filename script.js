...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('searchForm').addEventListener('submit', async function(event) {
      ...
      const url = 'https://jobs-api14.p.rapidapi.com/list?query=' + encodeURIComponent(searchInput) + '&location=Japan&distance=1.0&language=en_GB&remoteOnly=false&datePosted=month&employmentTypes=fulltime%3Bparttime%3Bintern%3Bcontractor&index=0';
      const options = {
          method: 'GET',
          headers: {
  ...
  locationButtons.forEach(button => {
      button.addEventListener('click', async function() {
          ...
          const url = `https://jobs-api14.p.rapidapi.com/list?query=%22%22&location=${encodeURIComponent(location)}&distance=1.0&language=en_GB&remoteOnly=false&datePosted=month&employmentTypes=fulltime%3Bparttime%3Bintern%3Bcontractor&index=0`;
          const options = {
              method: 'GET',
              headers: {
                  'X-RapidAPI-Key': '5fa6052638mshccab636a6bf1991p139232jsn54ffc2d42558',
      ...
      jobs_data.jobs.forEach(job => {
          ...
          const jobHTML = `
              <div class="job">
                  ...
                  <p><i>Company:</i> ${job.company}</p>
                  <p><i>Posted:</i> ${job.datePosted}</p>
                  <p><i>Location:</i> ${job.location}</p>
                  <p><i>Type:</i> ${job.employmentType}</p>
                  <!-- <p><i>Description:</i> ${truncatedDescription}</p> --> <!-- Display truncated description -->
              </div>
          `;
